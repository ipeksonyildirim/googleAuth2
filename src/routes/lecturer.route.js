...
router.get("/department/id=:id", async (req, res, next) => {
  let lecturers;
  try {
    lecturers = await Lecturer.find().where('department').equals(req.params.id).populate('user');
  }catch (err){
    const error = new HttpError(
  ...
  if(lecturers.length > 0){
    // for each lecturer get lecturer.user.name and lecturer.user._id
    let lecturer_names = [];
    for(let i = 0; i < lecturers.length; i++){
      lecturer_names.push({
        name: lecturers[i].user.name,
        id: lecturers[i]._id
      })
    }
    res.json({
    ...
    const error = new HttpError(
      'No lecturer found for this department.',
      404
    );
  }
})
